...
Reference mockups: [`front-end-design/`](front-end-design/) — `mockup-device.png`, `mockup-mig.png`,
`mockup-software.png`. They are **direction, not specification**. §7 records what is deliberately not taken
from them.
...
Two of these are measured, not matters of taste. They are listed first because they are the ones that make
the page look broken rather than plain.

**0.1 The chart palette fails on the surface it renders on.** `TimeSeriesPanel` uses eight hues that are the
*light-mode* steps of the reference palette, drawn on a dark panel. Run against the **current** panel
surface, `#161b22`:

```
$ node scripts/validate_palette.js "#2a78d6,#eb6834,#1baf7a,#eda100,#e87ba4,#008300,#4a3aa7,#e34948" \
      --mode dark --surface "#161b22"
  [FAIL] Lightness band     outside band: #eb6834, #eda100, #e87ba4, #4a3aa7
  [WARN] Contrast vs surface  below 3:1: #4a3aa7 (2.02)
```

Four of eight slots sit outside the dark lightness band, and the violet slot is at 2.02:1 — a series the
reader cannot see. Colours are also assigned by `SERIES_COLORS[datasets.length % 8]`, which is assignment by
*rank*: changing the GPU filter repaints every surviving series a different colour.

**0.2 Twenty-seven of the fifty-eight panels are dead, and say "No data in this range".** The entire
`GPU Software — eBPF` dashboard. Its expressions use `$pod`, `$__range` and `$__rate_interval`; the UI
substitutes only `$gpu`, so the literal text reaches Prometheus:

```
$ curl -sG .../api/query --data-urlencode \
    'q=sum(increase(ebpf_cuda_kernel_launch_calls_total{k8s_pod_name=~"$pod"}[$__range]))'
{"detail":"Prometheus returned 400: parse error: unexpected character in duration expression: '$'"}
```

Prometheus returns **400**. The renderers treat only `>= 500` as an error, so a rejected query is reported as
absence of data — the exact ambiguity [12 §3.1](12-monitoring-ui.md) exists to remove. No amount of styling
fixes a third of the UI being blank, so §5 and §6 are part of this work, not a follow-up.

**0.3 The rest is ordinary flatness.** Four unlabelled `<select>` elements of equal visual weight; rows as
browser-default `<details>` triangles with no panel counts; panel descriptions reachable only by hovering a
`title` attribute; Chart.js's in-canvas legend eating plot area; gridlines at `#30363d` nearly as strong as
the data.
...
**Changing any hex invalidates the result — re-run the validator, do not reason about it.**
...
| `$__rate_interval` | 22 panels | `max(step + scrapeInterval, 4 × scrapeInterval)`, Grafana's own rule. `scrapeInterval` is 30s, the widest interval in `deploy/a30-node/` — the widest is the safe choice, because a rate window narrower than the true scrape interval yields gaps |
...
## 6. Four empty states, not three

[12 §3.1](12-monitoring-ui.md) names three causes. A fourth is real and is what hid §0.2 for as long as it
did:

| Cause | How the UI knows | What it says |
|---|---|---|
| Unsupported on this hardware | `gpu_metric_supported` is `0` | "Not supported on this GPU" |
| Nothing running | 200, empty result | "No data in this range" |
| **Query rejected** | **Prometheus returned 4xx** | **"Query rejected" + the upstream reason** |
| Prometheus unreachable | 502/503 | "Prometheus unreachable" |

A malformed query is a *bug*, not an observation about the cluster, and must never render as absence of data.
Each state keeps a distinct colour from the status palette and a label — never colour alone.
