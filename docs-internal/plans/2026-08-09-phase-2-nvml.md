...
func TestAnnotationMatchedBySuffixNotExactPrefix(t *testing.T) {
	...
	for _, key := range []string{
		...
		got := DeviceUUIDFromAnnotations(map[string]string{key: "GPU-abc123,NVIDIA,4096,50:;"})
		if got != "GPU-abc123" {
			t.Fatalf("key %s gave %q", key, got)
		}
	}
}

func TestMissingOrUnparseableAnnotationYieldsEmpty(t *testing.T) {
	if got := DeviceUUIDFromAnnotations(nil); got != "" {
		t.Fatalf("nil annotations gave %q", got)
	}
	if got := DeviceUUIDFromAnnotations(map[string]string{"hami.io/vgpu-devices-allocated": "!!!"}); got != "" {
		t.Fatalf("garbage gave %q", got)
	}
}
...
Expected: FAIL — `undefined: DeviceUUIDFromAnnotations`
...
// DeviceUUIDFromAnnotations returns the first UUID-shaped token of the HAMi
// allocation annotation, or "".
//
// Matched by key SUFFIX because the prefix differs between HAMi variants.
// Deliberately tolerant of format drift: any parse failure yields "".
func DeviceUUIDFromAnnotations(annotations map[string]string) string {
	for key, value := range annotations {
		if !strings.HasSuffix(key, annotationSuffix) || value == "" {
			continue
		}
		first := value
		...
		if m := uuidRe.FindString(first); m != "" {
			return m
		}
	}
	return ""
...
func TestSamplesSharingANameShareALabelSet(t *testing.T) {
	// Differing label sets under one name are rejected by a pedantic registry
	// and would break the scrape.
	c := NewPrometheusAdapter([]Collector{stubCollector{rows: []Sample{
...
import (
	...
	"github.com/OWNER/monitoring-system/exporters/nvml/internal/collector"
)
...
func (d nvmlDevice) Processes() []collector.ProcSample {
	...
	// Catalog row 22. On a MIG-enabled PARENT this returns nothing useful,
	// which is why MIG instances are enumerated as their own handles.
	if procs, ret := d.handle.GetComputeRunningProcesses(); ret == nvml.SUCCESS {
		for _, p := range procs {
			byPID[p.Pid] = &collector.ProcSample{
				...
				MemoryBytes: float64(p.UsedGpuMemory),
			}
		}
	}
	...
	if !d.mig {
		if samples, ret := d.handle.GetProcessUtilization(0); ret == nvml.SUCCESS {
			for _, s := range samples {
				...
				if !ok {
					entry = &collector.ProcSample{PID: s.Pid}
					byPID[s.Pid] = entry
				}
...
//
// The binding carries both the current and the legacy spelling of this API, so
// no hand-written version shim is needed (docs-internal/04 § 1).
func (d nvmlDevice) eventReasons() map[string]bool {
	...
	bits := map[string]uint64{
		...
		"hw_slowdown":                 nvml.ClocksEventReasonHwSlowdown,
		"sync_boost":                  nvml.ClocksEventReasonSyncBoost,
		"sw_thermal_slowdown":         nvml.ClocksEventReasonSwThermalSlowdown,
		"hw_thermal_slowdown":         nvml.ClocksEventReasonHwThermalSlowdown,
		"hw_power_brake_slowdown":     nvml.ClocksEventReasonHwPowerBrakeSlowdown,
...
// MIG instances of any partitioned device. Per-process memory is only reported
// on the instance handle (catalog MIG row 17).
func discoverDevices() ([]nvmlDevice, error) {
	...
	for i := 0; i < count; i++ {
		...
		out = append(out, nvmlDevice{handle: handle, index: i, mig: migOn})
...
import (
	...
	"github.com/OWNER/monitoring-system/exporters/nvml/internal/alloc"
	"github.com/OWNER/monitoring-system/exporters/nvml/internal/collector"
)

// k8sAllocSource reads entitlement from the Kubernetes API: one code path for
// device-plugin, HAMi annotation and DRA, with no privileged kubelet mount
// (D-10).
...
func (s k8sAllocSource) Entitlements() []collector.Entitlement {
	...
	pods, err := s.client.CoreV1().Pods("").List(ctx, metav1.ListOptions{
		FieldSelector: "spec.nodeName=" + s.node,
	})
	if err != nil {
		slog.Warn("pod list failed", "err", err)
		return nil
	}

	deviceUUIDs := s.resourceSliceUUIDs(ctx)
	...
	for i := range pods.Items {
		...
		if uuid := alloc.DeviceUUIDFromAnnotations(pod.Annotations); uuid != "" {
			out = append(out, collector.Entitlement{
				GPUUUID: uuid, Namespace: pod.Namespace, Pod: pod.Name,
				Container: container, Source: "annotation",
			})
			continue
		}

		for _, ref := range pod.Spec.ResourceClaims {
			for _, device := range alloc.DevicesFromClaim(s.claimResults(ctx, pod, ref)) {
				if uuid, ok := deviceUUIDs[device]; ok {
					out = append(out, collector.Entitlement{
						GPUUUID: uuid, Namespace: pod.Namespace, Pod: pod.Name,
						Container: container, Source: "dra",
					})
				}
			}
		}
...
import (
	...
	"github.com/OWNER/monitoring-system/exporters/nvml/internal/cgroup"
	"github.com/OWNER/monitoring-system/exporters/nvml/internal/collector"
	"github.com/OWNER/monitoring-system/exporters/nvml/internal/podcache"
)
