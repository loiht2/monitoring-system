...
| **D-5** | Packaging | **Plain numbered YAML**, one directory per environment | Mirrors the platform's existing convention. **Superseded — see D-5a** |
| **D-5a** | Packaging (revised) | **Plain numbered YAML, a single `deploy/`** | The per-environment directories diverged by 0–4 lines of image tokens while duplicating ten manifests. The duplication cost more than the isolation bought: a fix applied to one copy and not the other is invisible. Environment differences are now `REPLACE_ME` tokens documented in `deploy/README.md` |
...
| [09 — Risks & open questions](09-risks-and-open-questions.md) | R-1…R-9, A-1…A-9, OQ-1…OQ-3, permanent limits, non-goals — **reference** |
| [10 — Metric support signal](10-metric-support-signal.md) | How a blank panel says *why* it is blank |
| [11 — Dashboards](11-dashboards.md) | The three dashboards and what each answers |
| [12 — Monitoring UI](12-monitoring-ui.md) | The native UI: panel spec, data flow, template variables |
| [13 — UI visual design](13-ui-visual-design.md) | Tokens, the validated palette, panel states, controls |
| [14 — Metric evaluation](14-metric-evaluation.md) | Proving every metric responds: the three outcomes and the harness |
| [15 — Image publishing](15-image-publishing.md) | Publishing the built images to GHCR without a registry credential |
