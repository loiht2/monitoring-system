...
Three dashboards, split by **what the reader is asking about**, not by which exporter produced the number.

| Dashboard | Answers | Sources behind it |
|---|---|---|
| **GPU Hardware — Device** | What is the whole card doing, who holds it, and is it healthy? | DCGM, NVML, `gpu_alloc_*`, `gpu_metric_supported` |
| **GPU Hardware — MIG** | The same questions, per MIG instance | DCGM (instance entities), NVML, `gpu_metric_supported` |
| **GPU Software — eBPF** | What is each pod asking CUDA to do, and where is it waiting? | eBPF CUDA tracing |

Device and MIG are separate dashboards rather than one with a filter because they describe **different
entities**: once MIG is enabled, DCGM stops reporting device-scope profiling fields for that card and
reports per-instance rows instead ([02 § 4](02-metric-catalog.md)). A single dashboard would have to show
readings whose scope silently changes with the selection, and an instance utilisation must never be summed
into a device total.
