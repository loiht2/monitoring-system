...
```
monitoring-system/
  README.md                   project front page and the layout map
  docs/                       user documentation — how to use the system
  specs/                      this specification — what it must do, and why
  exporters/
    nvml/
  ...
  dashboards/                 Grafana JSON — the source of truth for panels
  services/
    advanced-monitoring-api/  FastAPI: proxies Prometheus, serves the panel spec
    advanced-monitoring-ui/   Next.js: the native UI, one deployable pair with the API
  deploy/                     numbered YAML, dependency order, one directory (D-5a)
  scripts/                    dashboard contract checker, panel-spec generator, PromQL helpers
  test/
    test_*.py                 unit tests for the tooling; no cluster required
    loadgen/                  deliberate GPU load, applied by hand during validation
    evaluation/               metric coverage harness — needs a GPU node and live Prometheus
  .github/workflows/          image build and publish to GHCR
```

**One `deploy/` directory, not one per environment** ([D-5a](00-decisions.md)). The per-environment copies
diverged by nothing but image tokens while duplicating ten manifests, and a fix applied to one copy and not
the other is invisible. Environment-specific content is limited to image references, namespaces, node
selectors and storage, and is carried as `REPLACE_ME` tokens documented in `deploy/README.md`.
...
environment-specific value. The publishing mechanics — tags, workflow, package visibility — are specified in
[15](15-image-publishing.md) and not repeated here.
