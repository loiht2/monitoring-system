...
async def label_values(client, base: str, label: str,
    ...
    Unscoped, Prometheus answers from the whole retention window, so a device that was
    removed days ago is still listed. Measured on the validation cluster: a MIG instance
    deleted hours earlier still appeared, which would put a dead entry in the GPU
